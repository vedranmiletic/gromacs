...
{
    /*
     * *****Densdevel pointer to array of density values in slices and frame-blocks
     * Densdevel[*nsttblock][*xslices][*yslices][*zslices] Densslice[x][y][z] nsttblock - nr of
     * frames in each time-block bw  widths of normal slices
     *
...
static void interfaces_txy(real****                Densmap,
                           int                     xslices,
                           int                     yslices,
                           int                     zslices,
                           int                     tblocks,
                           ...
                           const gmx_output_env_t* oenv)
{
    ...
    t_interf ***  int1    = nullptr,
             ***int2      = nullptr; /*Interface matrices [t][x,y] - last index in row-major order*/
...
static void writesurftoxpms(t_interf***                      surf1,
                            t_interf***                      surf2,
                            int                              tblocks,
                            int                              xbins,
                            int                              ybins,
...
static void writeraw(t_interf***                      int1,
                     t_interf***                      int2,
                     int                              tblocks,
                     int                              xbins,
                     int                              ybins,
                     ...
                     const gmx_output_env_t*          oenv)
{
    ...
    try
    {
        ...
        settings.linePrefix("# ");
        gmx::printBinaryInformation(raw1, output_env_get_program_context(oenv), settings);
        gmx::printBinaryInformation(raw2, output_env_get_program_context(oenv), settings);
    }
...
{
    static const char* desc[] = { "[THISMODULE] reduces a two-phase density distribution",
    ...
    t_pargs pa[] = {
        { "-1d", FALSE, etBOOL, { &b1d }, "Pseudo-1d interface geometry" },
        { "-bw",
          FALSE,
          etREAL,
          { &binw },
        ...
        { "-axis", FALSE, etSTR, { &axtitle }, "Axis Direction - X, Y or Z" },
        { "-method", FALSE, etENUM, { meth }, "Interface location method" },
        { "-d1", FALSE, etREAL, { &dens1 }, "Bulk density phase 1 (at small z)" },
        { "-d2", FALSE, etREAL, { &dens2 }, "Bulk density phase 2 (at large z)" },
...
#define NFILE asize(fnm)
    ...
    if (bFourier)
    {
        ...
        if (spectra.size() != 2)
        {
            gmx_fatal(FARGS, "No or not correct number (2) of output-file-series: %td", spectra.ssize());
        }
